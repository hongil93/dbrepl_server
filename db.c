...
char* get_select_all()
{
    ...
    int buffer_size = 1024; // ?ç†?èô?òô?ç†?èô?òô ?? ??ç†?èô?òô
    ...
    result_buffer[0] = '\0'; // ?ç†?èô?òô ?ç†?èô?òô?ç†?èô?òô??ç†Ôø? ?ç†?èô?òô?ç†?èô?òô?

    while ((row = mysql_fetch_row(res)))
    {
        for (i = 0; i < num_fields; i++)
        {
            // ??ç†Ôø????ç†?èô?òô ?ç†?èô?òô ??êÑ? ?ç†?èô?òô? ?? ??ç†?èô?òô? ?ç†?èô?òô?ç†?èô?òô??ç†Ôø? ??
            int field_length = row[i] ? strlen(row[i]) : 4; // "NULL"?ç†?èô?òô ?ç†?èô?òô?ç†?èô?òô?ç†?èô?òô 4
            int required_length = strlen(result_buffer) + field_length + 2; // ?ç†?èô?òô?ç†?èô?òôÂ©? '\0' ??
...
//     //get db02 data csv
// 	fp = popen("mysql -h 10.0.2.4 -e \"SELECT * FROM eluon;\" repl_test01 > /home/khchoi/dbrepl_server/backup/db02_data.csv", "r");
// 	if (fp == NULL){
// 		printf("popen fail\n");
// 		return 0;
// 	}
...
char* compare_table(int db_index)
{
    ...
    const char *sql_load_data = "LOAD DATA LOCAL INFILE '/home/khchoi/dbrepl_server/backup/db02_data.csv' "
                                "INTO TABLE USER_TB "
...
void get_replication_on(int fd)
{
    ...
    // ??ç†?èô?òô??ç†?èô?òô ?ç†Ôø???ç†Ôø??
    printf("Result:\n%s\n", result_buffer);
    packet.header.type = REP_ON; // ??ç†?èô?òô??ç†?èô?òô ?????ç†?èô?òô ??ç†?èô?òô??ç†?èô?òô
    strncpy(packet.buf, result_buffer, BUF_SIZE - 1); // ÂØÉÍ≥å?Çµ Ë∏∞Íæ™?ùÅ ËπÇÎì≠Í∂?
    packet.header.length = strlen(packet.buf); // ??ç†?èô?òô??ç†?èô?òô Êπ≤Î™Ñ?î† ??ç†?èô?òô??ç†?èô?òô

    // ??ç†?èô?òô??ç†?èô?òô??ç†?èô?òô ??ç†?èô?òô??ç†?èô?òô
...
void get_replication_off(int fd)
{
    ...
    // ??ç†?èô?òô??ç†?èô?òô ?ç†Ôø???ç†Ôø??
    printf("Result:\n%s\n", result_buffer);
    packet.header.type = REP_OFF; // ??ç†?èô?òô??ç†?èô?òô ?????ç†?èô?òô ??ç†?èô?òô??ç†?èô?òô
    strncpy(packet.buf, result_buffer, BUF_SIZE - 1); // ÂØÉÍ≥å?Çµ Ë∏∞Íæ™?ùÅ ËπÇÎì≠Í∂?
    packet.header.length = strlen(packet.buf); // ??ç†?èô?òô??ç†?èô?òô Êπ≤Î™Ñ?î† ??ç†?èô?òô??ç†?èô?òô
...
void get_sql_insert_table(int fd, const char *query)
{
    ...
    // ??ç†?èô?òô??ç†?èô?òô ?ç†Ôø???ç†Ôø??
    printf("Result:\n%s\n", result_buffer);
    packet.header.type = SQL_INSERT; // ??ç†?èô?òô??ç†?èô?òô ?????ç†?èô?òô ??ç†?èô?òô??ç†?èô?òô
    strncpy(packet.buf, result_buffer, BUF_SIZE - 1); // ÂØÉÍ≥å?Çµ Ë∏∞Íæ™?ùÅ ËπÇÎì≠Í∂?
    packet.header.length = strlen(packet.buf); // ??ç†?èô?òô??ç†?èô?òô Êπ≤Î™Ñ?î† ??ç†?èô?òô??ç†?èô?òô
